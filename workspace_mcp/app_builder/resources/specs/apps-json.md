...
| `img`, `img_dark`, `img_light` | Recommended | Image URLs. Empty string `""` is fine. |
...
1. **Group `name` follows the literal `Group 1`, `Group 2`, ... pattern.** Custom names like `"symbol-group"` validate fine but fail silently — no widget joins the group at runtime.
